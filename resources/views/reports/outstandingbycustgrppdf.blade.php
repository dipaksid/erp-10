        ...
        <main>
            ...
            @foreach($data as $i => $rows)
                ...
                @if($karea=="" || $karea!=$rows->areacode || $kcsgp!=$rows->groupname)
					@if($kcsgp=="")
						<div>
							<div class="col">{{$rows->groupname}}</div>
							<div class="fclear"></div>
						</div>
					@php $ilop++; @endphp
					@if($ilop%$maxpg==0 && $ilop>0)
                    ...
                    @if($karea!="")
                        <div>
                            ...
                            <div class="col col6" style="border-top: solid 1px;">{{number_format($sum[$karea],2)}}</div>
                            <div class="fclear"></div>
                        </div>
                        @php $sum[$karea]=0; unset($sum[$karea]); $ilop++; @endphp
						@if($ilop%$maxpg==0 && $ilop>0)
							<div class="page-break"></div>
						@endif
                    @endif
					@if($kcsgp=="" || $kcsgp!=$rows->groupname)
						@if($kcsgp!="")
							<div>
								<div class="col col15">TOTAL {{$kcsgp}}:</div>
								<div class="col col6" style="border-top: solid 1px;">{{number_format($sum[$kcsgp],2)}}</div>
								<div class="fclear"></div>
							</div>
							@php $ilop++; @endphp
							@if($ilop%$maxpg==0 && $ilop>0)
                ...
                @php
                    $karea = $rows->areacode;
                    $kcsgp = $rows->groupname;
                    $sum[$karea] = (isset($sum[$karea]))?$sum[$karea]+$rows->out_amt:$rows->out_amt;
                    $sum[$kcsgp] = (isset($sum[$kcsgp]))?$sum[$kcsgp]+$rows->out_amt:$rows->out_amt;
                    $totsum+=$rows->out_amt;
                    $ilop++;
                @endphp
            @endforeach
            ...
            @if($kcsgp!="")
                <div>
                    ...
                    <div class="col col3"></div>
                    <div class="col col4"></div>
                    <div class="col col5">Total {{$kcsgp}}:</div>
                    <div class="col col6" style="border-top: solid 1px;">{{number_format($sum[$kcsgp],2)}}</div>
                    <div class="fclear"></div>
                </div>
            @endif
