...
@section('content')
    <div class="container-fluid">
        ...
        <form id="agentform" method="post" action="{{ action('App\Http\Controllers\AgentsController@update', $agent->id) }}">
            @csrf
            @method('PATCH')
            ...
            <div class="row form-group">
                <div class="col-3">
                    ...
                    <input type="text" seq="1" class="form-control enterseq" name="agentcode" value="{{ $agent->agentcode }}" maxlength="10" />
            ...
            <div class="row form-group">
                <div class="col-9">
                    ...
                    <input type="text" seq="2" class="form-control enterseq" name="name" value="{{$agent->name}}" maxlength="60" />
                ...
                <div class="col-3">
                    ...
                    <select class="form-control enterseq" seq="3" name="areas_id">
                        ...
                        @foreach ($data["area"] as $id => $rarea)
                            <option value="{{ $id }}" {{ (($id == $agent['areas_id']) ? "selected" : "") }}>{{ $rarea }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row form-group">
                <div class="col-3">
                    ...
                    <input type="text" class="form-control enterseq" seq="4" name="commrate" value="{{$agent->commrate}}" maxlength="8" />
                </div>
            </div>

            <a href="{{ action('App\Http\Controllers\AgentsController@index') }}" class="btn btn-secondary btn-xs">Back</a>
            <button type="submit" seq="5" class="btn btn-primary enterseq">Update</button>
